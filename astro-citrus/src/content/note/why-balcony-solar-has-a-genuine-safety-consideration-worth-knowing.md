---
title: "Why balcony solar has a genuine safety consideration worth knowing"
description: "Plug-in solar pushes power back through your outlets, which can stress older wiring. Here's the real risk and why UL 3700 certification matters."
publishDate: "2026-04-22T00:00:00Z"
---

Most US electrical wiring assumes power flows one direction: from the panel to the outlet. [Plug-in solar](/notes/balcony-solar-the-renters-first-real-shot-at-solar/) reverses that. When a balcony system pushes power back through a receptacle, it can potentially overload older wiring or interfere with ground-fault circuit interrupters (GFCIs).

This isn't a reason to avoid balcony solar — it's a reason to buy [certified hardware](/posts/plug-and-play-solar-panels/).

In early 2026, UL Solutions launched the [UL 3700 certification](/posts/ul-1741-sa-anti-islanding-plug-in-solar/) specifically for plug-in solar. A UL 3700 certified kit includes automatic power cut-off if you unplug mid-sun, grid outage protection so utility workers don't get shocked, and built-in overload protection to keep your circuits from running hot.

If you're buying a balcony solar kit: look for UL 3700. If a kit doesn't have it, that's the thing to ask about before plugging it into your home.
